...
export async function middleware(request: NextRequest) {
  ...
  if (
    ...
    pathname.includes('.') ||
    pathname.startsWith('/test-')  // Skip middleware for test pages
  ) {
    return NextResponse.next();
  }

  // Temporarily disable session checking to allow testing
  // TODO: Re-enable once Edge Runtime compatibility is fixed
  return NextResponse.next();
